import socket as sk
import sys
import clientDirectory.ClientLibrary as ClientLibrary
...
while True:
    ...
    try:
        if text.split([0]) == "get":
            ClientLibrary.get_client(sock, text.split([1]))
        elif text.split([0]) == "put":
            ClientLibrary.put_client(text.split([1]), sock, server_address)
        elif text.startswith == "list":
            ClientLibrary.list_client(sock,server_address)
